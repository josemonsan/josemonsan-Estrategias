...
## 14. Follow-up — data snapshot committed to `data/` (same day, 2026-09-17)

§13's `fetch_data.py` defaulted to writing into a gitignored `data/` directory (regenerable, not versioned). Ran `python3 fetch_data.py --universe universe_43.csv --out-dir data` for the full 2015-01-01 → today range and committed the output directly, at the user's request, rather than leaving it regenerate-only:

- `data/prices.csv` — 43-ticker daily OHLCV, 126,593 rows (~12 MB)
- `data/macro_monetary_base.csv` — FRED `BOGMBASE`, 199 monthly observations
- `data/earnings_dates.csv` — 4,161 earnings dates across 43 tickers

Removed `/data/` from `.gitignore` accordingly — this directory is now an intentional versioned snapshot, not a regenerate-only scratch output. Verified after pushing: `origin/master` and local `HEAD` point to the identical commit SHA, and the SHA-256 of each local file matches the SHA-256 of the corresponding blob in `origin/master` — byte-for-byte identical, not just "same commit message."

Practical effect: every fetch-dependent analysis in this file (§1-12) can now be re-run against this exact committed snapshot instead of re-downloading from Yahoo Finance/FRED — useful for reproducing a specific number without depending on any of those sources being up or unchanged. Re-running `fetch_data.py` will still overwrite `data/` with a fresh pull (prices update daily, earnings calendars get new future dates) — diff before committing if exact reproducibility of a past result matters more than freshness.
...
- §14 committed an actual snapshot to `data/` (verified byte-identical on `origin/master`) so this file's numbers can be reproduced against the exact data used, not just a same-methodology re-download — re-running `fetch_data.py` will refresh `data/` with newer prices/earnings, so diff before committing if reproducing a past result matters more than freshness.
